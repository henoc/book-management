...
import com.example.bookmanagement.repository.AuthorRepositoryTest.Companion
...
class BookRepositoryTest {
    ...
    fun `should save and retrieve book`() {
        val book = Book(null, "Test Book", 2021, 1)
    ...
    fun `should update book`() {
        val book = Book(null, "Original Title", 2021, 1)
        val savedBook = bookRepository.save(book)
    ...
    fun `should delete book`() {
        val book = Book(null, "Book to Delete", 2021, 1)
        val savedBook = bookRepository.save(book)
    ...
    fun `should find all books`() {
        val book1 = Book(null, "Book 1", 2021, 1)
        val book2 = Book(null, "Book 2", 2022, 2)
    ...
    fun `should find books by author id`() {
        val book1 = Book(null, "Book 1", 2021, 1)
        val book2 = Book(null, "Book 2", 2022, 1)
        val book3 = Book(null, "Book 3", 2023, 2)
        bookRepository.save(book1)
        bookRepository.save(book2)
        bookRepository.save(book3)

        val authorBooks = bookRepository.findByAuthorId(1)
        assertEquals(2, authorBooks.size)
        assertTrue(authorBooks.all { it.authorId == 1 })
    }
}